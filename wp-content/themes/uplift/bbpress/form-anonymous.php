...
<?php if ( bbp_current_user_can_access_anonymous_user_form() ) : ?>
	...
	<fieldset class="bbp-form">
		<legend><?php ( bbp_is_topic_edit() || bbp_is_reply_edit() ) ? _e( 'Author Information', 'uplift' ) : _e( 'Your information:', 'uplift' ); ?></legend>

		<?php do_action( 'bbp_theme_anonymous_form_extras_top' ); ?>

		<p>
			<label for="bbp_anonymous_author"><?php _e( 'Name (required):', 'uplift' ); ?></label><br />
		...
		<p>
			<label for="bbp_anonymous_email"><?php _e( 'Mail (will not be published) (required):', 'uplift' ); ?></label><br />
		...
		<p>
			<label for="bbp_anonymous_website"><?php _e( 'Website:', 'uplift' ); ?></label><br />
