    ...
    */
    ...
    if ( ! function_exists( 'sf_get_post_item' ) ) {
        function sf_get_post_item( $postID, $blog_type, $show_title = "yes", $show_excerpt = "yes", $show_details = "yes", $excerpt_length = "20", $content_output = "excerpt", $show_read_more = "yes", $fullwidth = "no" ) {
            ...
            if ( strpos( $post_excerpt, '[spb' ) !== false ) {
                $post_excerpt = "";
            }
            ...
            $download_text      = apply_filters( 'sf_post_download_text', __( "Download", 'uplift' ) );
            ...
            if ( $blog_type == "bold" ) {
                ...
                if ( $show_details == "yes" ) {
                    if ( $single_author && !$remove_dates ) {
                        $post_item .= '<div class="blog-item-details">' . sprintf( __( '<span>In %1$s</span> <time class="date" datetime="%2$s">%3$s</time>', 'uplift' ), $post_categories, $post_date_str, $post_date ) . '</div>';
                    } else if ( ! $remove_dates ) {
                        $post_item .= '<div class="blog-item-details">' . sprintf( __( '<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> <span>in %3$s</span> <time class="date" datetime="%4$s">%5$s</time>', 'uplift' ), $post_author, get_author_posts_url( get_the_author_meta( 'ID' ) ), $post_categories, $post_date_str, $post_date ) . '</div>';
                    } else if ( ! $single_author ) {
                        $post_item .= '<div class="blog-item-details">' . sprintf( __( '<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> <span>in %3$s</span>', 'uplift' ), $post_author, get_author_posts_url( get_the_author_meta( 'ID' ) ), $post_categories ) . '</div>';
                    }
                }
    ...
    if ( ! function_exists( 'sf_post_thumbnail' ) ) {
        function sf_post_thumbnail( $blog_type = "", $fullwidth = "no" ) {
            ...
            if ( $blog_type == "mini" ) {
                ...
                $gallery_size = 'sf-thumb-image';
            } else if ( $blog_type == "masonry" ) {
                if ( $sidebar_config == "both-sidebars" || $fullwidth == "yes" ) {
                    $item_class = "col-sm-3";
                } else {
                    $item_class = "col-sm-4";
                }
            ...
            if ( $thumb_type == "video" ) {
                ...
                if ( $thumb_type == "image" && $thumb_img_url == "" ) {
              	...
              	$image        = sf_aq_resize( $thumb_img_url, $thumb_width, $thumb_height, true, false );
              	$image_alt    = sf_get_post_meta( $image_id, '_wp_attachment_image_alt', true );			
//	            $image       = wp_get_attachment_image( $image_id, apply_filters( 'sf_image_post_thumbnail', 'blog-image' ), array(
//	//            	'title'	=> $image_title,
//	//            	'alt'	=> $image_title,
...
//	            ) );
                ...
                if ( $thumb_img_url != "" ) {
                    if ( $image ) {
                        //$item_figure .= '<div class="img-wrap">'.$image.'</div>';
        ...
        function sf_get_masonry_post( $postID, $thumb_type, $fullwidth, $show_title, $show_excerpt, $show_details, $show_read_more, $content_output, $excerpt_length ) {
            ...
            if ( $post_object['type'] == "post" ) {
                if ( $post_object['format'] == "standard" ) {
                    $post_item .= '<h6>' . __( "Article", 'uplift' ) . '</h6>';
                } else {
    ...
    if ( ! function_exists( 'sf_get_mini_post' ) ) {
        function sf_get_mini_post( $postID, $thumb_type, $fullwidth, $show_title, $show_excerpt, $show_details, $show_read_more, $content_output, $excerpt_length ) {
			...
			$mini_details = "";
			if ( $single_author && ! $remove_dates ) {
			    $mini_details .= '<div class="mini-item-details">' . sprintf( __( 'in %1$s / <time datetime="%2$s">%3$s</time> / %4$s comments', 'uplift' ),  $post_object['categories'], $post_object['date_str'], $post_object['date'], $post_object['comments'] ) . '</div>';
			} else if ( ! $remove_dates ) {
			    $mini_details .= '<div class="mini-item-details">' . sprintf( __( '<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> in %3$s / <time datetime="%4$s">%5$s</time> / %6$s comments', 'uplift' ), $post_object['author'], get_author_posts_url( get_the_author_meta( 'ID' ) ), $post_object['categories'], $post_object['date_str'], $post_object['date'], $post_object['comments'] ) . '</div>';
			} else if ( ! $single_author ) {
			    $mini_details .= '<div class="mini-item-details">' . sprintf( __( '<span class="author">By <a href="%2$s" rel="author" itemprop="author">%1$s</a></span> / %3$s / %4$s comments', 'uplift' ), $post_object['author'], get_author_posts_url( get_the_author_meta( 'ID' ) ), $post_object['categories'], $post_object['comments'] ) . '</div>';
			}
    ...
    if ( ! function_exists( 'sf_get_post_details' ) ) {
        function sf_get_post_details( $postID ) {
        	...
        	if ( $single_author && ! $remove_dates ) {
        	    $post_details .= '<div class="blog-item-details"><time class="post-date" datetime="' . $post_date_str . '">' . sprintf( __( '%1$s', 'uplift' ), $post_date ) . '</time></div>';
        	} else if ( ! $remove_dates ) {
        	    $post_details .= '<div class="post-item-details"><time class="post-date" datetime="' . $post_date_str . '">' . $post_date . '</time><span class="author"> ' . sprintf( __( 'by <a href="%2$s" rel="author" itemprop="author">%1$s</a>', 'uplift' ), $post_author, get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '</span></div>';
        	} else if ( ! $single_author ) {
        	    $post_details .= '<div class="post-item-details"><span class="author">' . sprintf( __( 'By <a href="%2$s" rel="author" itemprop="author">%1$s</a>', 'uplift' ), $post_author, get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '</span></div>';
        	}
    ...
    if ( ! function_exists( 'sf_get_recent_post_item' ) ) {
        function sf_get_recent_post_item( $post, $display_type = "bold", $excerpt_length = 20, $item_class = "" ) {
            ...
            $recent_post   = $recent_post_figure = $image_srcset = $link_config = $item_icon = "";
            $thumb_type    = sf_get_post_meta( $post->ID, 'sf_thumbnail_type', true );
            $thumb_image   = rwmb_meta( 'sf_thumbnail_image', 'type=image&size=full' );
            $thumb_video   = sf_get_post_meta( $post->ID, 'sf_thumbnail_video_url', true );
            ...
            if ( strpos($item_class, 'col-sm-4') !== false ) {
            ...
            if ( $thumb_type == "video" ) {
                ...
                if ( $image ) {
                	...
                	if ( $image_srcset ) {
                		$recent_post_figure .= '<div class="img-wrap">' . $image_srcset . '</div>';
                	} else {
    ...
    if ( ! function_exists( 'sf_get_campaign_item' ) ) {
        function sf_get_campaign_item( $item_class ) {
            ...
            if ( class_exists( 'ATCF_Campaigns' ) ) {
                ...
                $campaign_item .= '<span>' . __( "Funded", 'uplift' ) . '</span>';
                $campaign_item .= '</div>';
                $campaign_item .= '<div class="detail pledged">';
                $campaign_item .= '<data>' . $campaign->current_amount() . '</data>';
                $campaign_item .= '<span>' . __( "Pledged", 'uplift' ) . '</span>';
                $campaign_item .= '</div>';
                if ( ! $campaign->is_endless() ) {
                    $campaign_item .= '<div class="detail">';
                    $campaign_item .= '<data>' . $campaign->days_remaining() . '</data>';
                    $campaign_item .= '<span>' . _n( "Day to Go", "Days to Go", $campaign->days_remaining(), 'uplift' ) . '</span>';
                    $campaign_item .= '</div>';
                }
                $campaign_item .= '</div>';
                $campaign_item .= '</div>';
