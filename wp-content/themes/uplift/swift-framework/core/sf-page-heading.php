    ...
    */
    ...
    if ( ! function_exists( 'sf_page_heading' ) ) {
        function sf_page_heading() {
            ...
            $shop_page  = false;
            $page_title = $page_subtitle = $page_title_style = $page_title_overlay_effect = $fancy_title_image_url = $article_heading_bg = $article_heading_text = $page_heading_el_class = $page_heading_wrap_el_class = $page_design_style = $page_title_text_align = $woo_category_desc = "";
            ...
            if ( isset( $sf_options['default_show_page_heading'] ) ) {
...
//                }
            ...
            if ( isset($sf_options['minimal_checkout']) ) {
				if ( function_exists('is_checkout') && is_checkout() ) {
		        	...
		        	if ( $sf_options['minimal_checkout'] ) { ?>
		            	...
		            	<div class="minimal-checkout-return container"><a href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>"><?php _e("Return to cart", 'uplift'); ?></a></div>
                    ...
                    <div class="container">
                        ...
                        <div class="heading-text">
                            ...
                            <?php } else if ( is_archive() ) { ?>
                                ...
                                if ( is_tag() ) { ?>
                                    <h1 <?php echo $article_heading_text; ?>><?php _e( "Posts tagged with", 'uplift' ); ?>
                                        &#8216;<?php single_tag_title(); ?>&#8217;</h1>
                                    <?php /* If this is a daily archive */
                                } elseif ( is_day() ) { ?>
                                    <h1 <?php echo $article_heading_text; ?>><?php _e( "Archive for", 'uplift' ); ?> <?php the_time( 'F jS, Y' ); ?></h1>
                                    <?php /* If this is a monthly archive */
                                } elseif ( is_month() ) { ?>
                                    <h1 <?php echo $article_heading_text; ?>><?php _e( "Archive for", 'uplift' ); ?> <?php the_time( 'F, Y' ); ?></h1>
                                    <?php /* If this is a yearly archive */
                                } elseif ( is_year() ) { ?>
                                    <h1 <?php echo $article_heading_text; ?>><?php _e( "Archive for", 'uplift' ); ?> <?php the_time( 'Y' ); ?></h1>
                                    <?php /* If this is an author archive */
                                } elseif ( is_author() ) { ?>
                                    <?php $author = get_userdata( get_query_var( 'author' ) ); ?>
                                    <?php if ( class_exists( 'ATCF_Campaigns' ) ) { ?>
                                        <h1 <?php echo $article_heading_text; ?>><?php _e( "Projects by", 'uplift' ); ?> <?php echo esc_attr($author->display_name); ?></h1>
                                    <?php } else { ?>
                                        <h1 <?php echo $article_heading_text; ?>><?php _e( "Author archive for", 'uplift' ); ?> <?php echo esc_attr($author->display_name); ?></h1>
                                    <?php } ?>
                                    <?php /* If this is a paged archive */
                                } elseif ( isset( $_GET['paged'] ) && ! empty( $_GET['paged'] ) ) { ?>
                                    <h1 <?php echo $article_heading_text; ?>><?php _e( "Blog Archives", 'uplift' ); ?></h1>
                                <?php } else { ?>
                            ...
                            <?php } else if ( is_404() ) { ?>
                                ...
                                <h1 class="entry-title" <?php echo $article_heading_text; ?>><?php _e( "404", 'uplift' ); ?></h1>
    ...
    if ( ! function_exists( 'sf_post_split_heading_buttons' ) ) {
        function sf_post_split_heading_buttons() {
        	...
        	$blog_page   = $sf_options['blog_page'];
