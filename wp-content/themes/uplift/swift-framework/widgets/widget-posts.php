    ...
    */
    ...
    class sf_recent_posts extends WP_Widget {
                        ...
                        <?php } ?>
                            ...
                            <div class="recent-post-details">
                                ...
                                <?php if ( sf_theme_opts_name() == "sf_uplift_options" ) {
                                	...
                                	if ( ! $remove_dates ) {
                                	    echo '<div class="blog-item-details">' . sprintf( __( '<time datetime="%1$s">%2$s</time>', 'uplift' ), get_the_date('Y-m-d'), get_the_date() ) . '</div>';
                                	}
                                } else { ?>
	                                <span><?php printf( __( 'By %1$s on %2$s', 'uplift' ), $post_author, $post_date ); ?></span>
        ...
        function form( $instance ) {
            ...
            ?>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo __( 'Title:', 'uplift' ); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"
                       name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr($title); ?>"
                       class="widefat"/>
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"><?php echo __( 'Number of posts to show:', 'uplift' ); ?></label>
                <input id="<?php echo esc_attr($this->get_field_id( 'number' )); ?>"
                       name="<?php echo esc_attr($this->get_field_name( 'number' )); ?>" type="text"
                       value="<?php echo esc_attr($number); ?>" size="3"/>
            </p>
            <p>
                <label for="<?php echo esc_attr($this->get_field_id( 'category' )); ?>"><?php _e( 'Category', 'uplift' ); ?></label>
